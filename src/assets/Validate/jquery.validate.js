...
$.extend( $.validator, {
	...
	messages: {
		...
		isIDCard:"Please enter only digits.",
		loginName:"Please enter only digits."//
	},
	...
	prototype: {
		...
		//xxh获取含有中文的字符长度，后端数据库是UTF-8编码，汉字占三个字节
		getChineseLength:function(str){
			...
			}else if(typeof(str)=='object'&&typeof($(str).val())!='undefined'){
				str=$(str).val();
				for (var i=0; i<str.length; i++) {  
					if (str.charCodeAt(i)>127) {  
						len += 3;  
	...
	classRuleSettings: {
		...
		isIDCard:{isIDCard:true},//add by luo
		loginName:{loginName:true}//add by luo
	},
